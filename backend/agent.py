...
def llm_response(message, session_id, file_text=None, want_voice_reply=False):
    # 3. USE A SPAN TO GROUP THE ENTIRE CHAT REQUEST LIFECYCLE
    with logfire.span("llm_response for session {session_id}", session_id=session_id):
        messages = []
        if file_text:
            messages.append({
                "role": "system",
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                # Log warnings to Logfire if empty responses happen
                logfire.warn(f"Model '{MODEL}' returned an empty response (attempt {attempt}/{max_attempts})")

            except Exception as e:
                # Log exceptions cleanly into Logfire traces
                logfire.error(f"Model '{MODEL}' failed on attempt {attempt}: {e}")
...
def _emit_audio_event(sentence_text):
    with logfire.span("Generate Audio TTS"):
        try:
